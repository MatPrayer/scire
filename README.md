...
[![Version](https://img.shields.io/badge/version-0.16.0-6f7ce8?style=flat-square)](Cargo.toml)
...
- [x] **Artist page**: discography, singles/EPs and an "Appears on" section of the records they only guest on; every artist an album is credited to links to their own page, and the photo enlarges like a cover.
...
- [x] **Multi-library**: sidebar checkbox selector, all selected libraries merged into one sorted view — including the artist page, which the server itself cannot filter.
...
### Fullscreen cover size

**Settings → Appearance → Fullscreen cover size** sets how far the now-playing
cover grows on a window with room to spare — **Fixed**, **Medium**, **Large**
(default) or **Huge**. Fixed pins it to the size a window that only just holds
the overlay draws. The cover only ever grows into width the info card does not
want, so the controls keep their room whichever size is picked.
