...
- **Vi-mode keyboard navigation**: when `settings.vi_mode` is ON, `root.rs` routes keys to `handle_vi_key` and the legacy no-vi shortcuts (space/arrows/`[`/`]` = play/prev/next/nav) are disabled. Every content view implements `vi_cursor: Option<usize>` + `vi_move(delta, window, cx)` / `vi_activate(cx)` / `vi_clear(cx)`; new Content variants must be added to `content_vi_move`/`content_vi_activate`/`content_vi_tab` + `vi_clear_cursors` in `root.rs`. `[`/`]` mean album filter tabs in vi mode, nav back/forward in no-vi mode. Focus highlight goes through **one** helper, `ui::with_focus_cursor(id, el, focused, glow, cx)` — border always, fill/glow/entry animation only when `Settings::selection_glow` is on; `focus_glow`/`with_focus_animation` are private so no call site can bypass the setting. `anchor_scroll` stays at the call site (per-view handle, must run whether or not the glow does). Scroll-into-view via `ScrollAnchor` (nested rows) or `scroll_to_item(row, ScrollStrategy::Top)` (virtualized album grid — uniform_list has no FirstVisible). **Anchor scrolling must be driven from `render`, not from `vi_move`**: the anchor's origin is a paint old, and `scroll_to` applies at the next frame's start, so from a key handler it scrolls to the row the cursor just LEFT — the focused row lands one row above the viewport going up and the highlight vanishes. `ui::sync_focus_scroll(anchor, cursor, synced, window, cx)` (with a `vi_scroll_synced: Option<usize>` field) does it a frame after the focused row painted; its `window.refresh()` is what makes that frame draw. A focused text field stands the shortcuts down (`RootView::vi_typing`: "focus is not mine" in insert mode, every field asked by name in normal mode — gpui focuses buttons on mouse-down, so the wide test would kill j/k after the first click) — the handler *returns without claiming the key*, which is what lets gpui insert the character. Collapsed, the sidebar walk still includes the playlists: the cursor entering their range opens the rail's dropdown (`rail_playlists_open`, controlled `Popover`).
