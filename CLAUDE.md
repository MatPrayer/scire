...
**Scirè** — cross-platform (macOS + Linux) desktop music client for [Navidrome](https://www.navidrome.org/) servers, built with GPUI (Zed's UI framework) + [gpui-component](https://github.com/longbridge/gpui-component). Speaks Subsonic API v1.16.1 + OpenSubsonic; identifies as `scire` in the Subsonic `c` param. All three milestones are implemented: M1 (connect/browse/play), M2 (search/queue/playlists/star), M3 (multi-library selection, scrobbling, internet radio, shares, transcoding settings, theming, OS media keys via souvlaki, artwork cache eviction) — plus the ported legacy features: waveform seek bar, global search bar, infinite-scroll album grid with persisted sort, artist bio/images via `getArtistInfo2`, lyrics, fullscreen now-playing overlay, mouse back/forward navigation, configurable start page. See `README.md` for the user-facing feature list and Linux build deps.
...
- **`crates/app`** — the gpui binary (package name `scire`). Layers:
  ...
  - `config.rs` — settings TOML via `directories::ProjectDirs`; password in OS keyring (`keyring` crate), plaintext-in-settings fallback only when keyring fails. Also owns the cache dirs (artwork, waveform peaks) and persisted UI prefs (`default_page`, `album_sort`, selected `library_ids` — legacy single `library_id` is migrated on load).
