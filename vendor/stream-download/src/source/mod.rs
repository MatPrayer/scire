    ...
    W: StorageWriter,
{
    ...
    /// Wake a reader waiting on a position the writer has now passed.
    ///
    /// LOCAL PATCH (Scire): factored out of `write` so the prefetch path can
    /// use it too. A seek restarts the prefetch at the seek target, and the
    /// prefetch path never notified — so a reader waiting on a position inside
    /// that new prefetch window stayed parked until some *later* ordinary
    /// write crossed it, which for a seek to the end of a file means after the
    /// whole file has downloaded. That is every m4a whose `moov` index sits
    /// after the audio: the decoder cannot be built until then.
    fn notify_if_position_reached(&mut self, new_position: u64) {
    ...
    async fn handle_prefetch(
        ...
        let Some(bytes) = bytes else {
            ...
            self.notify_if_position_reached(position);
        ...
        self.notify_if_position_reached(stream_position);
    ...
    async fn write(&mut self, bytes: Bytes) -> io::Result<u64> {
        ...
        while written < bytes.len() {
            ...
            self.notify_if_position_reached(new_position);
            if new_written == 0 {
