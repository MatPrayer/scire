...
impl LineWrapper {
                ...
                let item_width = match candidate {
                    WrapBoundaryCandidate::Char { character: c } => {
                        ...
                        if Self::is_word_char(c) {
                            ...
                            // Scirè patch: closing punctuation never starts a line, so
                            // `mode).` or `song;` can't leave `).` / `;` on its own.
                            if c != ' '
                                && first_non_whitespace_ix.is_some()
                                && !(prev_c != ' ' && Self::clings_to_previous(c))
                            {
                                last_candidate_ix = ix;
                                last_candidate_width = width;
                            }
    ...
    /// Punctuation that closes what comes before it, and so must not be
    /// wrapped onto a line of its own (Scirè patch).
    pub(crate) fn clings_to_previous(c: char) -> bool {
        matches!(
            c,
            ')' | ']' | '}' | ';' | '!' | '?' | '"' | '…' | '’' | '”' | '»' | '›'
                | '。' | '、' | '，' | '；' | '：' | '！' | '？' | '）' | '」' | '』' | '】' | '》'
...
mod tests {
    ...
    #[test]
    fn test_closing_punctuation_stays_on_its_word() {
        let mut wrapper = build_wrapper();
        // `)` overflows at 7: the break goes before `bbb`, not before `)`.
        assert_eq!(
            wrapper
                .wrap_line(&[LineFragment::text("aaa bbb) cc")], px(72.))
                .collect::<Vec<_>>(),
            &[Boundary::new(4, 0)],
        );
        assert_eq!(
        ...
        // After a space, `"` opens a quote and may start the line.
        assert_eq!(
            wrapper
                .wrap_line(&[LineFragment::text("aaa b \"cc\"")], px(72.))
                .collect::<Vec<_>>(),
            &[Boundary::new(6, 0)],
        );
    }
